...
class ReservationsController extends Controller
{
    ...
    public function showDuplicationForm($reservation_id)
    {
        Reservation::where('reservation_id', $reservation_id)->firstOrFail();
        $customers = Customer::where('deleted_at', null)->get();

        return view('reservations/duplicate', compact('reservation_id', 'customers'));
    }
    ...
    public function duplicate(Request $request)
    {
        ...
        while (true) {
            $new_reservation_id = substr(md5(rand()), 0, 15);
            $reservation_id_found = Reservation::where('reservation_id', $new_reservation_id)->first();
            if (!$reservation_id_found) {
                break;
            }
        }
        Reservation::create([
        ...
        return redirect(route('reservation.showAll'))->with('success',  __('view.reservationDuplicated', ['id' => $new_reservation_id]));
    }
    ...
    public function filter(Request $request)
    {
        ...
        if (!$request->data) { // if There is no searching data return all hotels
            ...
            // Get reservations
            $reservations = Reservation::where(function ($query) use ($data) {
                $query->where('deleted_at', null);
                $query->where('reservation_id', 'like', '%' . $data . '%');
            })
                ->orWhere(function ($query) use ($data) {
                    $query->where('deleted_at', null);
                    $query->where('customer_id', 'like', '%' . $data . '%');
                })
                ->orWhere(function ($query) use ($data) {
                    $query->where('deleted_at', null);
    ...
    public function exportPdf($reservationId){
        ...
        $view = view('reservations' . '/' . $lang . '_pdf', [
            'lang' => $lang,
        ...
        if ($lang == 'en') {
            $pdf->setFooter(
               ...
               <svg class="icon" width="15" height="15" viewBox="0 -15 24 38" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                ...
                <span style="font-size: 10px"><a href="' . $settings['pdf_website_url'] .
                    '" target="_blank" style="text-decoration: none; color: #000">www.formula.com.sa</a></span> 
                ...
                <a href="' . $settings['pdf_facebook'] .
                    '"><svg  class="icon facebook" width="15" height="15" viewBox="0 -15 24 38" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path fill="white" stroke="white" d="M0 0h24v24H0z" />
                ...
                <a href="' . $settings['pdf_twitter'] .
                    '"><svg class="twitter icon" width="15" height="15" viewBox="0 -15 24 38" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path fill="white" stroke="white" d="M0 0h24v24H0z" />
                ...
                <a href="' . $settings['pdf_instagram'] .
                    '"><svg class="instagram icon" viewBox="0 -15 132.004 132" width="15" height="15">
                    <defs>
                        <linearGradient id="b">
                ...
                <a href="' . $settings['pdf_snapchat'] .
                    '"><svg class="snapchat icon" width="15" height="15" viewBox="147.353 39.286 514.631 514.631">
                ...
                <a href="' . $settings['pdf_linkedin'] .
                '"><svg class="linked-in icon" viewBox="0 0 48 48" width="15px" height="15px">
        ...
        $pdf->WriteHTML($view);
        $pdf->SetDirectionality($lang == 'en' ? 'ltr' : 'rtl');
